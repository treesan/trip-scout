...
3. 单日 tab：路线段用实线连接（区别于步行/打车的虚线），当天用单一颜色
4. 总览 tab：按天配色连线（所有日程段汇总展示，颜色与右下角图例一致），单击点位显示日程归属
5. 集成"📍 专属地图"按钮 — 用户点击即可在手机高德App中打开专属地图
6. 输出到 `output/{trip-name}/index.html`
